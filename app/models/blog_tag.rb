class BlogTag < Tag
  ...
  def self.summary
    select("blog_tags.id, blog_tags.name, COUNT(blog_entries.id) AS entries_count")
      .joins(:entries)
  ...
  def self.public_listing
    select("blog_tags.id, blog_tags.name, COUNT(blog_entries) AS entries_count, MAX(blog_entries.updated_at) AS updated_at")
      .joins(:entries)
